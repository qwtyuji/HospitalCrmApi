...
class CreatePatientsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('patients', function (Blueprint $table) {
            ...
            $table->unsignedInteger('doctor_id')->comment('医生')->nullable();
            $table->unsignedInteger('user_id')->comment('用户');
            $table->unsignedInteger('hospital_id')->comment('医院');
            $table->unsignedInteger('department_id')->comment('科室')->nullable();
            $table->unsignedInteger('disease_id')->comment('疾病')->nullable();
            $table->unsignedInteger('depart_id')->comment('部门');
            $table->unsignedInteger('media_id')->comment('媒体');
            $table->string('area')->comment('地区')->nullable();
            $table->string('order_time')->comment('预约时间')->nullable();
            $table->unsignedInteger('status')->comment('状态')->default(0);
            $table->timestamps();
        });
